...
//
//  https://github.com/tobihagemann/JXLabel
//
...
- (BOOL)hasFadeTruncating {
    return self.fadeTruncatingMode != JXLabelFadeTruncatingModeNone;
}
...
- (CGFloat)strokeSizeDependentOnStrokePosition {
    switch (self.strokePosition) {
        case JXLabelStrokePositionCenter:
...
- (void)drawRect:(CGRect)rect {
    ...
    BOOL needsMask = hasGradient || (hasStroke && self.strokePosition == JXLabelStrokePositionInside) || hasInnerShadow;
    ...
    if (hasStroke) {
        ...
        if (self.strokePosition == JXLabelStrokePositionOutside) {
            // Create an image from the text.
            image = CGBitmapContextCreateImage(context);
        } else if (self.strokePosition == JXLabelStrokePositionInside) {
            // Clip the current context to alpha mask.
            CGContextClipToMask(context, rect, alphaMask);
        }
    ...
    if (hasFadeTruncating) {
        ...
        CGImageRef linearGradientImage = [self linearGradientImageWithRect:rect fadeHead:self.fadeTruncatingMode & JXLabelFadeTruncatingModeHead fadeTail:self.fadeTruncatingMode & JXLabelFadeTruncatingModeTail];
...
- (UIEdgeInsets)fittingTextInsets {
    ...
    if (hasStroke) {
        switch (self.strokePosition) {
            case JXLabelStrokePositionOutside:
